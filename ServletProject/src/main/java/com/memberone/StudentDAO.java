...
public class StudentDAO {
	...
	public void updateMember(StudentVO vo) {
		...
		try {
			con = getConnection();
			String sql = "update student set pass=?, phone1=?, phone2=?, phone3=?, email=?, zipcode=?, address1=?, address2=? where id=?";  //정보수정하는 커리문
			pstmt = con.prepareStatement(sql);
	...
	/*
	 *   회원탈퇴 버튼을 클릭하면 데이터베이스에서 회원데이터가 삭제되어야 함.
	 *   회원삭제를 처리하는 메소드를 구현함. 
	 */
	...
	public int deleteMember(String id, String pass) {
		...
		ResultSet rs = null;
		int result = -1;
		//db에서 pass를 가져옴 , 입력한 pass와 dbpass를 비교함
		String dbPass="";
		try {
			...
			if(rs.next()) {
				dbPass=rs.getString("pass"); //db에서 가져온 값을 변수에 담음
				if(dbPass.equals(pass)) {  // 비밀번호가 일치할 경우
